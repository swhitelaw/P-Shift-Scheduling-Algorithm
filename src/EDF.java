...
public class EDF {
    ...
    static EDF [] o_p = new EDF[i_counter];
        ...
        for(int j=0;j< i_counter;j++)	
                {
                    if((o_p[j].i_duration>0)&&(a[j]==1))
                     {
                        if((o_p[j].i_deadline!=-1)&&((o_p[loc].i_deadline>o_p[j].i_deadline)||
                        		(o_p[loc].i_deadline==-1)))
                        	loc=j;
                        else if((o_p[loc].i_priority>o_p[j].i_priority)&&
                        		((o_p[j].i_deadline!=-1)))
                            loc=j; 
                    	/*if (cost(j)<cost(loc)){
                    		loc = j;
	    ...
	    for(int i = 1; i<=6; i++){   	
		    ...
		    for(int j=0; j<trials; j++){
			    ...
			    for(int i = 0; i<10; i++){
			        ...
			        //output();
			        System.out.println(s_sequence);
			        // System.out.println("Avaerage Wait Time: ");
